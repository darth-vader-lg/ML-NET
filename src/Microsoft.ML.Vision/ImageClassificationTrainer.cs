...
namespace Microsoft.ML.Vision
{
            ...
            ImageClassificationModelParameters>
    {
        ...
        #region EncodeByteAsString incompatibility workaround for TensorFlow 2.4.1
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Naming", "MSML_GeneralName:This name should be PascalCased", Justification = "Just for having the same name as Tensorflow")]
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Naming", "MSML_ParameterLocalVarName:Parameter or local variable name not standard", Justification = "Just for having the same name as Tensorflow")]
        private static unsafe ulong TF_StringEncode(byte* src, ulong src_len, byte* dst, ulong dst_len, Status status)
        {
            var sz = TF_StringEncodedSize(src_len);
            if (sz < src_len) {
                status.SetStatus(TF_Code.TF_INVALID_ARGUMENT, "src string is too large to encode");
                return 0;
            }
            if (dst_len < sz) {
                status.SetStatus(TF_Code.TF_INVALID_ARGUMENT, $"dst_len ({dst_len}) too small to encode a {src_len}-byte string");
                return 0;
            }
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Naming", "MSML_ParameterLocalVarName:Parameter or local variable name not standard", Justification = "Just for having the same name as Tensorflow")]
        private static unsafe void StringEncode(byte* src, ulong src_len, byte* dst)
        {
            dst = EncodeVarint64(dst, src_len);
            System.Buffer.MemoryCopy(src, dst, src_len, src_len);
        }

        private static unsafe byte* EncodeVarint64(byte* dst, ulong v)
        {
            const int b = 128;
            byte* ptr = dst;
            while (v >= b) {
        ...
        private static int VarintLength(ulong v)
        {
            int len = 1;
            while (v >= 128) {
                v >>= 7;
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Naming", "MSML_GeneralName:This name should be PascalCased", Justification = "Just for having the same name as Tensorflow")]
        private static ulong TF_StringEncodedSize(ulong len)
        {
            return (ulong)VarintLength(len) + len;
        }

        private static Tensor OldEncodeByteAsString(VBuffer<byte> buffer)
        {
            // Replaced the buggy calls to the c_api.TF_StringEncodedSize and TF_StringEncode.
            // Since the commit https://github.com/tensorflow/tensorflow/commit/24f835217fd27f21141ff0254a2b93ea3cfd7b6c
            // many string functions are dismissed but TensorFlow.NET continue to call them, generating
            // an invalid entry point exception.
            ...
            unsafe {
                fixed (byte* src = buffer.GetValues())
                    TF_StringEncode(src, (ulong)length, (byte*)(tensor + sizeof(Int64)), size, status);
            }
        ...
        {
            var t = new Tensor(buffer.GetValues().ToArray(), Array.Empty<long>(), TF_DataType.TF_STRING);
            return t;
        }
